...
<div
	class="mb-8 w-full max-w-4xl rounded-xl border-2 border-amber-500/50 bg-stone-900/80 p-6 shadow-xl backdrop-blur-sm"
>
		...
		<div class="flex items-center gap-3">
			<h3 class="text-xl font-bold text-amber-200">
				...
				{#if metaProgress['custom_champion']}
					<span class="ml-2 text-sm font-bold text-emerald-400"
						>(Lv. {metaProgress['custom_champion'].level})</span
					>
				{/if}
			</h3>
		</div>
		<div
			class="rounded-full border border-amber-900/50 bg-stone-950 px-4 py-2 font-mono text-lg font-bold"
		>
			Points Available:
			<span
				class={availablePoints > 0
					? 'text-amber-400'
					: availablePoints === 0
						? 'text-emerald-400'
						: 'text-red-400'}
	...
	<!-- Role Selection -->
	<div class="mb-6 rounded-lg border border-stone-700/50 bg-stone-800/50 p-4">
		...
		{:else}
			<div class="flex flex-wrap gap-3">
				{#each ['Warrior', 'Mage', 'Rogue', 'Cleric'] as role}
					<button
						class="btn px-4 py-2 text-sm {customRole === role
	...
	<div class="grid grid-cols-1 gap-8 md:grid-cols-2">
				...
				>
					...
					<div class="flex items-center gap-3">
						...
						{#if !isSaved}<button
								class="btn btn-secondary h-8 w-8 !p-0"
								onclick={() => customMana++}
								disabled={customMana >= 99 || availablePoints < COST_MANA}>+</button
							>{/if}
					</div>
				</div>
			</div>
		...
		<div>
			...
			<div class="space-y-4">
				<div class="rounded-lg border border-stone-700/50 bg-stone-800/50 p-4">
					<label
						for="customActiveSkill"
						class="mb-2 block text-xs font-bold text-stone-400 uppercase">Active Skill</label
					>
					{#if isSaved}
						<div
							class="w-full rounded border border-amber-900/50 bg-stone-900 p-2 text-sm text-stone-200"
						>
					...
					{:else}
						<select
							id="customActiveSkill"
							class="w-full rounded border border-amber-900/50 bg-stone-900 p-2 text-sm text-stone-200 outline-none focus:border-amber-500"
							bind:value={customActiveSkill}
						>
					...
					{#if customActiveSkill}
						...
						{#if selectedActive && selectedActive.roleAffinity === customRole}
							<p class="mt-1 text-[11px] font-bold text-emerald-400">
								✨ Role Match: {selectedActive.boostedEffect}
							</p>
						{:else if selectedActive}
							<p class="mt-1 text-[11px] font-bold text-red-400">
				...
				<div class="rounded-lg border border-stone-700/50 bg-stone-800/50 p-4">
					<label
						for="customPassiveSkill"
						class="mb-2 block text-xs font-bold text-stone-400 uppercase">Passive Skill</label
					>
					{#if isSaved}
						<div
							class="w-full rounded border border-amber-900/50 bg-stone-900 p-2 text-sm text-stone-200"
						>
							{#if customPassiveSkill}
								{@const selectedPassive = passiveSkillsList.find(
									(s) => s.name === customPassiveSkill
								)}
								{selectedPassive?.icon}
								{selectedPassive?.name}
							{/if}
						</div>
					{:else}
						<select
							id="customPassiveSkill"
							class="w-full rounded border border-amber-900/50 bg-stone-900 p-2 text-sm text-stone-200 outline-none focus:border-amber-500"
							bind:value={customPassiveSkill}
						>
					...
					{#if customPassiveSkill}
						...
						{:else if selectedPassive}
							<p class="mt-1 text-[11px] font-bold text-amber-500">
	...
	<div
		class="mt-6 flex flex-col items-start justify-between gap-4 border-t border-amber-900/30 pt-4 md:flex-row md:items-center"
	>
		{#if !isSaved}
			<button
				class="btn px-6 py-2 transition-colors {isCustomValid
					? 'bg-amber-600 text-white shadow-[0_0_15px_rgba(217,119,6,0.3)] hover:bg-amber-500'
