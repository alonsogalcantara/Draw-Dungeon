...
{#if skillCheck}
	...
	{#if !rolled}
		...
		<div class="mb-2 flex flex-col items-center gap-3">
			<div
				class="flex h-16 w-16 animate-pulse items-center justify-center rounded-full border-2 border-amber-600/30 bg-amber-900/20 text-3xl"
			>
				🎲
			</div>
			<button
				class="btn btn-primary text-md w-full max-w-[200px] px-6 py-2 tracking-wider"
		...
		<div class="mb-2 flex flex-col items-center gap-3">
			...
			<div class="mt-2 text-center">
				{#if success}
					...
					<h3 class="mt-1 text-xl font-black tracking-wider text-emerald-400">Success!</h3>
				{:else}
					<div class="text-4xl">❌</div>
					<h3 class="mt-1 text-xl font-black tracking-wider text-red-400">Failed!</h3>
				{/if}
			</div>
			...
			{#if dungeonDie !== null}
				<div class="mt-1 rounded-lg bg-stone-800/60 px-3 py-1.5 text-center text-xs text-stone-300">
					...
					{#if skillCheck.outcome}
						<span class="mt-1 block text-[10px] text-stone-400">{skillCheck.outcome}</span>
					{/if}
				</div>
			{/if}

			<button class="btn btn-primary mt-2 w-full px-8 py-3" onclick={() => resolveSkillCheck()}>
				Continue
			</button>
		</div>
	{/if}
