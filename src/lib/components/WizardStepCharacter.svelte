...
<div class="mb-8 grid w-full max-w-6xl grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
	{#each CHARACTERS as char (char.id)}
		...
		<div
			...
			tabindex="0"
			class={[
				'card group relative cursor-pointer rounded-xl border-2 p-5 text-left transition-all duration-300 hover:scale-[1.02]',
		...
		>
			<!-- Header -->
			<div class="mb-3 flex items-start justify-between border-b border-amber-900/30 pb-3">
				<div>
					<h2 class="text-xl font-bold text-amber-100">{char.name}</h2>
					<p class="mt-1 text-sm tracking-wider text-amber-500/70 uppercase">{char.className}</p>
				</div>
			</div>

			<!-- Stats -->
			<div class="mb-3 grid grid-cols-3 gap-2 text-center text-xs">
				<div class="rounded border border-emerald-900/50 bg-emerald-900/30 px-2 py-1" title="Level">
					<span class="font-bold text-emerald-400">Lv.</span>
					{metaProgress[char.id]?.level ?? 1}
				</div>
				<div class="rounded bg-red-900/30 px-2 py-1" title="Starting HP">
					<span class="text-red-400">❤️</span>
					{char.startingStats.hp +
						((metaProgress[char.id]?.level ?? 1) - 1) * 5 +
						(metaProgress[char.id]?.statUpgrades?.hp ?? 0)}
				</div>
				<div class="rounded bg-amber-900/30 px-2 py-1" title="Starting Food">
					<span class="text-amber-400">🍖</span>
					{char.startingStats.food + (metaProgress[char.id]?.statUpgrades?.food ?? 0)}
				</div>
				<div class="rounded bg-yellow-900/30 px-2 py-1" title="Starting Gold">
					<span class="text-yellow-400">💰</span>
					{char.startingStats.gold + (metaProgress[char.id]?.statUpgrades?.gold ?? 0)}
				</div>
			...
			<div class="mb-3 space-y-2">
				...
				{#each char.skills as skill (skill.name)}
					<div class="flex items-start gap-2">
						<span
							class="badge mt-0.5 shrink-0 rounded px-1.5 py-0.5 text-[10px] font-medium {skillTypeBadge[
								skill.type
			...
			{#if char.lore}
				<p
					class="mt-auto border-t border-amber-900/20 pt-3 text-xs leading-relaxed text-stone-500 italic"
				>
					{char.lore}
				</p>
			{/if}
						...
						<button
							...
							onclick={(e) =>
								handleSpendVP(
									char.id,
									'hp',
									char.startingStats.hp + ((metaProgress[char.id]?.level ?? 1) - 1) * 5,
									e
								)}>+1 HP</button
						>
						<button
							class="rounded border border-amber-500/30 bg-amber-900/40 px-1 py-1 text-[10px] text-amber-200 transition-colors hover:bg-amber-800/60"
							onclick={(e) => handleSpendVP(char.id, 'food', char.startingStats.food, e)}
							>+1 Food</button
						>
						<button
							class="rounded border border-yellow-500/30 bg-yellow-900/40 px-1 py-1 text-[10px] text-yellow-200 transition-colors hover:bg-yellow-800/60"
							onclick={(e) => handleSpendVP(char.id, 'gold', char.startingStats.gold, e)}
							>+1 Gold</button
						>
						<button
							class="rounded border border-blue-500/30 bg-blue-900/40 px-1 py-1 text-[10px] text-blue-200 transition-colors hover:bg-blue-800/60"
							onclick={(e) => handleSpendVP(char.id, 'armor', char.startingStats.armor, e)}
							>+1 Armor</button
						>
					</div>
				</div>
