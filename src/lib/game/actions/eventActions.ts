...
export function performSkillCheck(reason?: string) {
	...
	game.skillCheck.diceResults = Array(game.characterDiceCount).fill({
		type: 'character',
		value: 0
	});
	game.skillCheck.dungeonDieResult = 1;
	game.skillCheck.poisonDieResult = game.poisoned
		? {
				type: 'poison',
				value: 0,
				isStar: false,
		...
		: null;
	game.skillCheck.curseDieResult = game.cursed
		? {
				type: 'curse',
				value: 0,
				isStar: false,
...
export function handleItemRoom(action: 'take' | 'ignore') {
	...
	if (action === 'take') {
		if (card.cost) {
			...
			else if (card.cost.stat === 'food' && game.food >= card.cost.value)
				game.loseFood(card.cost.value);
...
export function handInMission(missionId: string) {
	...
	target.reward.effects.forEach((e) => {
		...
		if (e.stat === 'armor') game.gainArmor(e.value);
		if (e.stat === 'food') {
			if (e.value > 0) game.gainFood(e.value);
			else game.loseFood(Math.abs(e.value));
		}
	});
...
export function useCharacterSkill(skillName: string) {
	...
	if (!roleMatch) {
		game.addLog(
			`Your class (${game.selectedCharacter?.className}) cannot effectively use ${skillName}.`,
			'system'
		);
		return;
	}
